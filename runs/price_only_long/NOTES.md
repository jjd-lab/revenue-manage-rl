...
| `sac_optimize1d` | SAC (`n_envs=1`) | **optimize_1d** (`n_grid=41`) | 150 000 | 42 | ~32 min |
...
| price_only_sac_optimize1d@150k | price_only | 1,071,553 | 0.873 | 1267 | **0.00** | 0.433 | 818,132 |
...
| Does **price_only SAC (optimize_1d)** beat `bc_sac_final` on **score**? | **NO** (−52.4k; 818.1k vs 870.6k) |
...
**Bottom line:** Price-only RL is a **clear improvement over classical baselines** and roughly **ties the prior joint `rl_best`** (PPO slightly above, SAC slightly below), with a **safer oversell profile**. It does **not** close the gap to **BC→SAC final**, which still leads on revenue/score by filling harder (paying for that with 40% oversell episodes).
...
- `artifacts/price_only_long/rl_sac_optimize1d.zip` ← SAC final @150k
- `artifacts/price_only_long/rl_sac_optimize1d_best.zip` ← EvalCallback best (shaped; early)
- Only `rl_ppo_analytic.zip` is retrainable from a shipped config (README § Model checkpoints); the SAC rows cannot be regenerated
