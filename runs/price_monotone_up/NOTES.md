...
**Constraint:** `direction: up`. `mode: clamp` clamps. `mode: penalty` does not; the wrapper subtracts `penalty * (violation_dollars / 40)` from the step reward.
...
**Path:** `python scripts/explain_rl_best.py --config configs/experiment_monotone_up_sac.yaml --model artifacts/price_monotone/monotone_up_clamp/best/best_model.zip --out runs/price_monotone_up/explain`
...
| clamp, frozen checkpoint | **2,077,976** | **+44,712** | [20,945, 66,164] | **0** |
| clamp, retrained | 1,923,106 | −110,158 | [−146,418, −73,687] | **0** |
...
| Does `mode: clamp` remove the markdown? | **YES** — zero charged decreases, frozen or retrained |
...
The penalty weight that won the selection still buys neither the guarantee nor the frozen arm's score. It lands between the two clamp arms, about 82k below the reference, and marks down on all 30 nights.
