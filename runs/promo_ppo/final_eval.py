...
def main() -> None:
    cfg_promo = load_config(str(ROOT / "configs" / "experiment_price_only_promo_ppo.yaml"))
    ...
    def env_promo():
        return make_env(cfg_promo, use_held_out=True)
    ...
    print("=== Baselines (price_only + analytic SL, no promo) ===", flush=True)
    for name, policy in [
    ...
    keep(
        run_checkpoint(
            "promo_ppo@200k",
            env_promo,
            ROOT / "artifacts/promo_ppo/rl_promo_ppo.zip",
            "ppo",
            episodes=EPISODES,
        )
    )
    promo_best = ROOT / "artifacts/promo_ppo/rl_promo_ppo_best.zip"
    if promo_best.exists():
        keep(run_checkpoint("promo_ppo_bestckpt", env_promo, promo_best, "ppo", episodes=EPISODES))
    keep(
        run_checkpoint(
            "pace_ppo@200k",
            env_pace,
    ...
    write_comparison(
        ...
        results,
        all_eps,
        title="Early-promo + pace price-only PPO — held-out comparison",
        episodes=EPISODES,
        preamble=[
            "Training: `early_promo` price post-process + `pace_reward` + `soft_day_upweight`. "
